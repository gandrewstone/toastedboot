...
/* 20100309: Andrew Stone: quadruple this time.  Now that the boot jumps direct to the app on
   power up the time is not as important.  But setting this larger makes the reset button timing a lot easier
   for noobs. */
...
#define SIG3	0x06
#define PAGE_SIZE	0x40U	//64 words
#define RESET_RAM_CUBBY ((uint16_t *) 2046)

#elif defined __AVR_ATmega328P__
#define SIG2	0x95
#define SIG3	0x0F
#define PAGE_SIZE	0x40U	//64 words
#define RESET_RAM_CUBBY ((uint16_t *) 4094)
...
/* function prototypes */
void putch(char);
void putNch(const char* vals, uint8_t len);
char getch(void);
void getNch(uint8_t);
...
const char devId[] = { 0x14,SIG1,SIG2,SIG3,0x10};
...
int main(void)
{
	...
	*/
	...
	flash_led(NUM_LED_FLASHES + bootuart); //,500,100,80);
#else
	flash_led(NUM_LED_FLASHES); //,500,100,80);
#endif
...
#if 0
void flash_led2(uint8_t count,uint8_t predelay, uint8_t ontime,uint8_t offtime)
{
	_delay_ms(predelay);
