...
all:
	@echo "Please specify make targets:"
	@echo "  Bootloaders:           atmega168 atmega328p atmega328p_20mhz atmega644p"
	@echo "  Write Fuses and Flash: atmega168_isp atmega328p_isp atmega644p_isp"


atmega168: TARGET = atmega168
atmega168: MCU_TARGET = atmega168
atmega168: CFLAGS += '-DMAX_TIME_COUNT=F_CPU>>4' '-DADABOOT=2' '-DWATCHDOG_MODS'
...
atmega328p_20mhz: TARGET = atmega328_20mhz
atmega328p_20mhz: MCU_TARGET = atmega328p
atmega328p_20mhz: CFLAGS += '-DMAX_TIME_COUNT=((((uint32_t)F_CPU)>>3)+(((uint32_t)F_CPU)>>4))' '-DBAUD_RATE=57600' '-DADABOOT=2' '-DWATCHDOG_MODS' '-DTOASTED_BOOT'
atmega328p_20mhz: AVR_FREQ = 20000000L 
atmega328p_20mhz: LDSECTION  = --section-start=.text=0x7800
atmega328p_20mhz: toastedBoot_328p_20mhz.hex

atmega328p_isp: atmega328
atmega328p_isp: TARGET = toastedBoot_328p_16mhz
...
clean:
	echo rm -rf *.o *.elf *.lst *.map *.sym *.lss *.eep *.srec *.bin *.hex

%.lst: %.elf
	@echo objdump: $(OBJDUMP) -h -S $< > $@
	$(OBJDUMP) -h -S $< > $@

%.hex: %.elf Makefile
	@echo objcopy: $(OBJCOPY) -j .text -j .data -O ihex $< $@
